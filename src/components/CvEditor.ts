...
export class CvEditor {
  ...
  private field(key: string, labelText: string, type: string, placeholder = ""): HTMLDivElement {
  ...
  private async pickPhoto() {
    ...
    if (data) { this.photoSrc = data; this.photoImg.src = data; }
  }
  ...
  private async submit() {
    ...
    const data: Omit<Member, "id" | "createdAt" | "updatedAt"> = {
      firstName:  this.val("firstName"),
      lastName:   this.val("lastName"),
      email:      this.val("email"),
  ...
  loadMember(member: Member) {
    ...
    Object.keys(this.fields).forEach((k) => {
      ...
      if (this.fields[k]) this.fields[k].value = typeof val === "string" ? val : "";
    });

    if (member.photo) { this.photoSrc = member.photo; this.photoImg.src = member.photo; }
